...
class Joc:
    ...
    def __init__(self, tabla=None):
        if tabla is not None:
            ...
            self.matr = [Joc.GOL for i in range(self.NR_COLOANE ** 2)]

    def final(self):
        rez = elem_identice(self.matr[0:3]) or elem_identice(self.matr[3:6]) or elem_identice(self.matr[6:9]) or \
              elem_identice(self.matr[0:9:3]) or elem_identice(self.matr[1:9:3]) or elem_identice(self.matr[2:9:3]) or \
              elem_identice(self.matr[0:9:4]) or elem_identice(self.matr[2:8:2])
        if rez:
            return rez
