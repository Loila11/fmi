...
int run_command(char **this_command, const char *target) {
    pid_t pid = fork();

    if (pid < 0) {
        return -1;
    } else if (pid == 0) {
        if (execv(target, this_command) < 0 ) {
            perror(NULL);
            return errno;
        }
    } else {
...
int set_command(char **this_command, const char *prefix) {
    char *target = (char *) malloc(MAX_CAPACITY * sizeof(char));
    strcpy(target, prefix);
    strcat(target, this_command[0]);
    run_command(this_command, target);
    free(target);
}
...
void read_input(char shell_prompt[], unsigned long int *length) {
    char *s = readline(shell_prompt);
    add_history(s);

    char *path = strtok(s, " \n");
    while (path != NULL) {
        strcpy(command[*length], path);
        (*length) ++;
        path = strtok(NULL, "  \n");
    }
...
void run_put_get(int dest_type, int src_type, unsigned long int src, unsigned long int dest, char *function) {
    ...
    if (src_type == 0) {
        set_command(command, ".");  // Source is a file
        return ;
    }
    char *aux = (char*) malloc(MAX_LENGTH * sizeof(char));
...
void format_destination(unsigned long int src, unsigned long int dest) {
    if (file_or_folder_dbxcli(command[dest]) == 1) {
        char *p = strrchr(command[src], '/');
        if (p == NULL) {
            strcat(command[dest], "/");
...
int main() {
    command = (char **) malloc (MAX_CAPACITY * sizeof(char*));
    for (int i = 0; i < MAX_CAPACITY; i ++) {
    ...
    while(1) {
        ...
        read_input(shell_prompt, &length);
