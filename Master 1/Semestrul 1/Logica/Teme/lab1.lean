...
-/
def applyFunction {α β : Type} : α → (α → β) → β := fun (a : α) (f : α → β) => f a
...
-/
def me : Student := {name := "Miruna", group := 1}
...
def swap {α β : Type} : Prod α β → Prod β α := fun (a, b) => (b, a)
...
/-
  ...
  Note that in the `succ n` pattern we don't use `n` in the definition, 
...
-/
def sub? : Nat → Nat → Option Nat := fun (n : Nat) (m : Nat) => 
...
def identity_implicit {a : Type} (x : a) := x
def identity' {a : Type} : a -> a := fun x : a => x

#eval identity Nat 7
#eval identity _ 7
#eval identity_implicit 7
